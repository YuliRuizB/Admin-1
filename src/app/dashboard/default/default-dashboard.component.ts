...
export class DefaultDashboardComponent implements OnInit, OnDestroy {
  ...
  createLiquidacion() {
    ...
    if (validTo == "") {
      ...
      if (typePayment == "") {
        ...
        // create record normally  // tranfsferencia
        //  TODO  
    ...
    if (resultValidations) {
      ...
      if (this.validateForm.valid) {  //TODO
        ...
        if (this.userlevelAccess != "3") {
          ...
          let status = this.currentUserSelected.status;             
          if (status == "preRegister") {
            this.userService.updateUserPreRegister(this.currentUserSelected.uid, "active");
          }
  ...
  submitEditForm(): void {
    ...
    if (this.newCustomerIdEditMode != "" && !this.resetClientEditInfo) {
      this.msg.error("Al cambiar de  cliente es necesario asignar una ruta y parada acorde al cliente.");
      validForm = false;
    }
    if (userName == "") {
    ...
    if (customerId == "") {
      this.msg.error(" El cliente es un valor requerido, favor de validar");
      validForm = false;
    }
    if (defaultRoute == "") {
